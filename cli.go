...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Skibidicord")
	var updateFlag = flag.Bool("repair", false, "Repair Skibidicord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Skibidicord")
	...
	if *versionFlag {
		fmt.Println("Skibidilotl Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Skibidilotl' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Skibidicord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Skibidicord will likely not work.")
}
